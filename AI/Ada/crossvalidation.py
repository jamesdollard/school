...
for v in range(10):
    ...
    print("Setosa solution: " + str(am.fit(trainData, trainL)))
    successValue = am.predictionAccuracy(testData, testL)
    print("Results: " + str(successValue) + "/15" )

    testL = np.where(testLabels == 'Iris-versicolor', 1, -1)   # versicolor
    trainL = np.where(trainLabels == 'Iris-versicolor', 1, -1)
    print("Versicolor solution: " + str(am.fit(trainData, trainL)))
    successValue = am.predictionAccuracy(testData, testL)
    print("Results: " + str(successValue) + "/15" )
    ...
    print("Virginica solution: " + str(am.fit(trainData, trainL)))
    successValue = am.predictionAccuracy(testData, testL)
    print("Results: " + str(successValue) + "/15\n" )
